...
public final class System {
    ...
    private static PrintStream getOutput() {
        try {
            String url = System.getProperty("java.lang.System.out");
            if (url == null) {
                url = "debug:";
            }
//Native.print("Initializing System.out to "); Native.println(url);
    ...
    public static void arraycopy(Object src, int srcPos, Object dst, int dstPos, int length) {
        ...
        ClassBase srcElmt = srcClass.elementType;
        ClassBase dstElmt = dstClass.elementType;

        if (srcElmt.isPrimitive() || dstElmt.isPrimitive()) {
            if (srcElmt != dstElmt) {
                throw new ArrayStoreException(srcElmt.className + " != " + dstElmt.className);
            }
        } else {
            if (!srcElmt.isAssignableTo(dstElmt)) {
                ...
                for (int i = 0 ; i < length ; i++) {
                    ...
                    if (item != null && !item.getClass().isAssignableTo(dstElmt)) {
                        throw new ArrayStoreException();
                    }
